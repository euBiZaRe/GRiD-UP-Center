...
// Extracted as a proper named sub-component to avoid React hook count issues
...
const TeamCard: React.FC<TeamCardProps> = ({ team, count, onSelect }) => (
  <div
    ...
    <div className="absolute inset-x-0 bottom-0 p-6 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
      <div className="flex items-center justify-between mb-2">
        ...
        {team.id !== 'solo' && team.id !== 'gridUp_practice' && (
          <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border ${count > 0 ? 'bg-accent/10 border-accent/30' : 'bg-white/5 border-white/10 opacity-40'}`}>
            <Users size={10} className={count > 0 ? 'text-accent' : 'text-gray-500'} />
            <span className={`text-[10px] font-black ${count > 0 ? 'text-accent' : 'text-gray-500'}`}>
              {count}
            </span>
          </div>
        )}
...
const TeamSelector: React.FC<TeamSelectorProps> = ({ onSelect }) => {
  ...
  useEffect(() => {
    ...
    onValue(teamsRef, (snapshot) => {
      ...
      Object.entries(data).forEach(([teamId, teamData]: [string, any]) => {
        if (teamData?.drivers) {
          newCounts[teamId] = Object.values(teamData.drivers || {}).filter((d: any) =>
  ...
  const topRow    = TEAMS.filter(t => t.id !== 'solo' && t.id !== 'gridUp_practice');
  const bottomRow = TEAMS.filter(t => t.id === 'solo' || t.id === 'gridUp_practice');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col items-center justify-center p-8 animate-in fade-in duration-1000 select-none">
      ...
      <div className="absolute top-0 right-0 h-8 flex items-center z-50">
        <button onClick={() => window.electron && window.electron.send('window-control', 'minimize')} className="h-full px-4 flex items-center justify-center text-gray-500 hover:bg-white/10 hover:text-white transition-colors" style={{ WebkitAppRegion: 'no-drag' } as any}>
          <Minus size={14} strokeWidth={3} />
        </button>
        <button onClick={() => window.electron && window.electron.send('window-control', 'maximize')} className="h-full px-4 flex items-center justify-center text-gray-500 hover:bg-white/10 hover:text-white transition-colors" style={{ WebkitAppRegion: 'no-drag' } as any}>
          <Square size={12} strokeWidth={3} />
        </button>
        <button onClick={() => window.electron && window.electron.send('window-control', 'close')} className="h-full px-4 flex items-center justify-center text-gray-500 hover:bg-status-error hover:text-white transition-colors" style={{ WebkitAppRegion: 'no-drag' } as any}>
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex gap-6 justify-center">
          {topRow.map(team => (
            <TeamCard key={team.id} team={team} count={teamCounts[team.id] || 0} onSelect={onSelect} />
          ))}
        </div>
